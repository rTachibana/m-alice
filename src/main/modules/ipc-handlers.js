  ...
  ipcMain.handle('process-image', async (event, options) => {
    try {
      ...
      if (options.applyWatermark && options.watermarkPath) {
        watermarkPath = resolveWatermarkPath(options.watermarkPath);
      ...
      const processingOptions = {
        ...
        watermarkOpacityMin: options.watermarkOpacityMin || 0.05,
        // HTMLのmin属性から取得した最小値
        invertWatermark: options.invertWatermark || false,
        enableOutline: options.enableOutline,
        watermarkSize: options.watermarkSize,
        outlineColor: options.outlineColor,
        resize: options.resize,
        noiseLevel: parseInt(options.noiseLevel) / 7,
        // 0-7の値を0-1の範囲に変換
        noiseTypes: options.noiseTypes || ['gaussian', 'dct'],
        // ノイズタイプを追加
        // ロゴ位置の設定を追加
        ...
        fakeMetadataType: options.fakeMetadataType,
        addNoAIFlag: options.addNoAIFlag,
        // 出力形式設定を追加
      ...
      // Python スクリプトを実行
      const pythonProcess = spawn(config.pythonExePath, [path.join(config.appRoot, 'src', 'backend', 'process.py'), tempInputPath, outputPath, optionsJson]);
...
const resolveWatermarkPath = watermarkPath => {
  // 絶対パスの場合はそのまま使用
  const isAbsolutePath = path.isAbsolute(watermarkPath);
  if (isAbsolutePath && fs.existsSync(watermarkPath)) {
