...
const extractMetadataTag = (metadata, tagName) => {
  ...
  for (const section in metadata) {
    for (const tag in metadata[section]) {
      if (tagsToSearch.some(t => tag.toLowerCase().includes(t.toLowerCase()) || t.toLowerCase().includes(tag.toLowerCase()))) {
...
const detectAIGeneration = metadata => {
  ...
  if (software) {
    const aiSoftwarePatterns = ['stable diffusion', 'midjourney', 'dall-e', 'comfyui', 'deepdream', 'neural', 'gan', 'generative', 'ai generated'];
  ...
  for (const section in metadata) {
    for (const tag in metadata[section]) {
      ...
      if (tag.toLowerCase().includes('prompt') || value.includes('prompt:') || value.includes('negative prompt:') || value.includes('steps:') && value.includes('sampler:') || value.includes('cfg scale:')) {
        confidence += 40;
